...
/**
 * +html.tsx — Web HTML shell, rendered once at build time.
 *
 * Critical jobs:
 *  1. Apply dark/light CSS vars BEFORE first paint → no flash of wrong colors.
 *  2. Set `--css-interop-darkMode` via setProperty so NativeWind reads it correctly.
 *  3. Apply .dark class to <html> from localStorage before any React renders.
 */
  ...
  return (
    <html lang="en">
      <head>
        ...
        {/* ── Gluestack CSS vars + page bg — applied before any JS runs ─── */}
...
html.dark {
  ...
  --color-indicator-error: 185 28 28;
        ...
        {/* ── Dark mode + NativeWind — runs synchronously before first paint ── */}
        <script dangerouslySetInnerHTML={{ __html: `(function(){try{
          // Tell NativeWind to use class-based dark mode (no leading space = critical)
          document.documentElement.style.setProperty('--css-interop-darkMode','class dark');
          // Apply dark class from localStorage or system preference
          var stored=localStorage.getItem('theme');
          var sysDark=window.matchMedia('(prefers-color-scheme:dark)').matches;
          if(stored==='dark'||(stored!=='light'&&sysDark)){
